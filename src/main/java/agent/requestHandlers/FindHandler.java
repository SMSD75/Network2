...
public class FindHandler {
    ...
    public String doAction(String request, String rootIp, int rootListenPort) {
        ...
        try {
            ...
            findedIp.add(response);
            if(response == null) {
                findedIp = findIp(parsedRequest.get(0), parsedRequest.get(1), rootIp, rootListenPort, null);
                cacheData(parsedRequest.get(0), parsedRequest.get(1), findedIp.get(0), findedIp.get(1), timeOfNowInSec(), cacheLines);
            }
        } catch (IOException e) {
    ...
    private void cacheData(String domain, String time, String ip, String validationTime, Long date, ArrayList<String> cacheLines) throws IOException {
        ...
        for (int counter = 0; counter < cacheLines.size(); counter++) {
            ...
            if (parsedLine.get(0).equals(domain) && parsedLine.get(1).equals(time)) {
                parsedLine.set(2, ip);
                System.out.println("i find that : " + date.toString());
    ...
    private ArrayList<String> findIp(String domain, String time,  String rootIp, int rootListenPort, String method) throws IOException {
        ...
        String finalIp = null;
        String validatoinTime = null;
        ArrayList<String> findedData = new ArrayList<String>();
        ...
        if (responseMethod.equals("recursive")) {
            ...
            validatoinTime = findTimeValue(jsonObject);
        }
        else {
            // so it's iterative
            String nextDnsServerPort = FindNextDnsServerPort(jsonObject);
            findedData = findIp(domain, time, "localhost", Integer.parseInt(nextDnsServerPort), "iterative");
        }

        jsonIOHandler.close();
        findedData.add(finalIp);
        findedData.add(validatoinTime);
        return findedData;
    }

    protected  String findTimeValue(JSONObject jsonObject){
        return JsonHandler.getValue(jsonObject, "time");
    }
